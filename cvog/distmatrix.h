 ...
 * @Last Modified Time: 2024-12-07 6:26:45
 */
...
struct MdistNoName {
  ...
  MdistNoName() : ng(0){};
  MdistNoName(int, double d0 = 0.0);
  MdistNoName(const MdistNoName&);

  void _setdist(size_t, size_t, double);
  void _addupdist(size_t, size_t, double);
  ...
  void addupdist(size_t, size_t, double);
  void setdist(size_t, double);

  // reset distance
  void resetDist();
  void resetDist(double);
  ...
  // for plus
  void operator+=(const MdistNoName&);
...
class Mdist : public MdistNoName {
  vector<string> name;

public:
  Mdist() = default;
  void init(const vector<string> &, double d0 = NAN, bool chk = true);
  void init(size_t n, double d0 = NAN);
