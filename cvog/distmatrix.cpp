 ...
 * @Last Modified Time: 2022-12-27 18:02:18
 */

#include "distmatrix.h"

/********************************************************************************
 * @brief Functions for Base Class MdistNoName
 * 
 * @param ng number of genome of distance matrix
 * @param dist distance
 ********************************************************************************/
MdistNoName::MdistNoName(int n, double d0) : ng(n) {
  dist.resize(msize(), d0);
};

MdistNoName::MdistNoName(const MdistNoName& org):ng(org.ng), dist(dist){};


void MdistNoName::resize(size_t n) {
  ng = n;
  dist.resize(n * (n - 1) / 2);
};

// option on sigle item
double MdistNoName::getdist(size_t i, size_t j) const {
  if (i < j) {
    if (j >= ng) {
...
double MdistNoName::_getdist(size_t i, size_t j) const {
  return dist[i + (j - 1) * j / 2];
};

pair<size_t, size_t> MdistNoName::getIndex(size_t ndx) const {
  if (ndx >= dist.size()) {
...
void MdistNoName::setdist(size_t i, size_t j, double d) {
  if (i < j) {
    if (j >= ng) {
      cerr << "Error: the index out of matrix" << endl;
      exit(4);
...
void MdistNoName::_setdist(size_t i, size_t j, double d) {
  dist[i + j * (j - 1) / 2] = d;
};

void MdistNoName::setdist(size_t i, double d) { dist[i] = d; };

void MdistNoName::_addupdist(size_t i, size_t j, double d) {
  dist[i + j * (j - 1) / 2] += d;
};

void MdistNoName::addupdist(size_t i, size_t j, double d) {
  if (i < j) {
    if (j >= ng) {
      cerr << "Error: the index out of matrix" << endl;
      exit(4);
...
// reset distance 
void MdistNoName::resetDist(double d0) {
  for (auto &d : dist)
    d = d0;
}

// reinitial the distance to NAN
void MdistNoName::resetDist() {
  for (auto &d : dist)
    d = NAN;
}

// global options
size_t MdistNoName::size() const { return ng; };

size_t MdistNoName::msize() const { return ng * (ng - 1) / 2; };

size_t MdistNoName::capacity() const {
  return dist.capacity() * sizeof(double) + sizeof(ng);
};


int MdistNoName::chkNAN(const vector<size_t> &ndx,
...
int MdistNoName::chkAllNAN(vector<pair<size_t, size_t>> &nandist) const {
...
int MdistNoName::nNAN(size_t i) const {
  int n(0);
  size_t j = 0;
  for (; j < i; ++j) {
    if (std::isnan(_getdist(j, i)))
...
bool MdistNoName::isNAN(size_t i, size_t j) const {
  return std::isnan(getdist(i, j));
};

bool MdistNoName::isNAN(size_t m) const { return std::isnan(dist[m]); };

//.... has NAN in the ith line
bool MdistNoName::hasNAN(size_t i) const {
  size_t j = 0;
  for (; j < i; ++j) {
    if (std::isnan(_getdist(j, i)))
      return true;
...
// for plus
void MdistNoName::operator+=(const MdistNoName& b) {
...
/********************************************************************************
 * @brief Function for Matrix of Distance with Genome Name
 * 
 * @param name vector for genome name 
 ********************************************************************************/

void Mdist::init(const vector<string> &nmlist, double d0, bool chk) {
  ng = nmlist.size();
  name = nmlist;
  dist.resize(msize(), d0);
  if (chk)
...
void Mdist::init(size_t n, double d0) {
  ng = n;
  name.resize(n);
  dist.resize(msize(), d0);
};
...
int Mdist::chkNAN(const vector<string> &gname,
  ...
  vector<size_t> ndx;
  name2ndx(gname, ndx);
  return MdistNoName::chkNAN(ndx, nandist);
};
